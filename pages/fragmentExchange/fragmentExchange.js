...
Page({
  ...
  getResultList(type) {
    ...
    request.exchangeList(this.data.page, 10, this.data.keywords, selectId).then((res) => {
  ...
  openDetail(e) {
    ...
    wx.navigateTo({ url: `/pages/chipGoodDetail/chipGoodDetail?id=${id}` })
    // wx.navigateTo({ url: `/pages/fragmentExchangeConfirm/fragmentExchangeConfirm?id=${id}` })
  },
  /**
   * 生命周期函数--监听页面加载
   */
