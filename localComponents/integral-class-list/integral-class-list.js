  ...
  methods: {
    ...
    async firstGetContent() {
      try {
        ...
        const res = await request.integralList({ page: 1, pageSize: 10, typeId: typeId })
        this.setData({
    ...
    async scrollLowerGetList() {
      if (this.data.page <= this.data.totalPage) {
        let typeId = this.data.activeClassId === 'all' ? '' : this.data.activeClassId
        const res = await request.integralList({ page: this.data.page, pageSize: 10, typeId: typeId })
